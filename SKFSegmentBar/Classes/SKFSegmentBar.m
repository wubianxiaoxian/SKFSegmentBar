...
#import "UIView+SKFSegmentBar.h"
#define kMinMargin 30
@interface SKFSegmentBar ()
{
    // 记录最后一次点击的按钮
    UIButton *_lastBtn;
}
...
@property (nonatomic, strong) SKFSegmentBarConfig *config;

@end
@implementation SKFSegmentBar
#pragma mark   ------------- 接口 --------------
+ (instancetype)segmentBarWithFrame: (CGRect)frame {
    SKFSegmentBar *segmentBar = [[SKFSegmentBar alloc] initWithFrame:frame];
...
- (SKFSegmentBarConfig *)config {
    if (!_config) {
        _config = [SKFSegmentBarConfig defaultConfig];
    }
    return _config;
}
